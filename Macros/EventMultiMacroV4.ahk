...
Routes := Map(
    ...
    "EventZoneToVoid", "sc:[115,223]|spl:X|sc:[141,236]|r:[0%Q10&20%S300]|wt:2000|spl:TpButton"
)
...
CreationMap := Map(
    "Main", {Title:"Multi-Macro", Video:"", Description:"EVENT VERSION`n`nA all around afk-grinding macro`nF3 : Start`nF6 : Pause`nF8 : Stop/Close Macro`n`nDebug is saved into `"Storage\MultiMacroV4Debug`"", Version:Version, DescY:250, MacroName:"Multi-Macro", IncludeFonts:false, MultiInstancing:true},
    "Settings", [
    ...
    if MacroTogglesMap["AutoDaycare"] or MacroTogglesMap["AutoEmpower"] {
        if not BooleanValueMap["UserIsPastRebirth9"] {
            if SubPosition = "Void" or CurrentZone = 0 {
                ...
                RouteUser(Routes["EventZoneToVoid"])
            }
